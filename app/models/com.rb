    ...
    if bot
      if bot.check_com_limit < 39
        ...
        if bot_piar_groups["response"]
          if bot_piar_groups["response"]["items"].any?
            ...
            response = Net::HTTP.post_form(uri, {"owner_id" => "-#{bot_piar_group}",
                "offset" => 2,
                "count" => 1,
                "filter" => "all",
                "access_token" => bot.access_token,
            ...
            if response["response"]
              ...
              self.make(bot_id) unless post_id.present?
            end
            ...
            if response["error"]
                ...
                response = Net::HTTP.post_form(uri, {"user_id" => bot.task.user.admin.vk_id,
                  "message" => "бот № #{bot.id} юзера #{bot.task.user.name} не получает посты со стены #{response["error"]}",
                  "access_token" => bot.access_token,
                  "v" => "5.62"})
            end
            sleep 1
            ...
            if response["error"]
                uri = URI.parse("https://api.vk.com/method/messages.send")
                response = Net::HTTP.post_form(uri, {"user_id" => bot.task.user.admin.vk_id,
                  "message" => "бот № #{bot.id} юзера #{bot.task.user.name} не постит в группу #{response["error"]}",
                  "access_token" => bot.access_token,
                  "v" => "5.62"})
            end

            if response["response"]
              comment_track = bot.task.comment_trakings.build(comment_id: comment.id,
                bot_id: bot.id)
              unless comment_track.save
                uri = URI.parse("https://api.vk.com/method/messages.send")
                response = Net::HTTP.post_form(uri, {"user_id" => bot.task.user.admin.vk_id,
                  "message" => "бот № #{bot.id} юзера #{bot.task.user.name} не постит",
                  "access_token" => bot.access_token,
                  "v" => "5.62"})
                sleep rand(1..2)
              end
