class Like < ActiveRecord::Base
  def self.make(bot_id)
    ...
    if bot
      if bot.check_like_limit < 399
        ...
        if bot.task.groups.empty?
          ...
          response = Net::HTTP.post_form(uri, {"group_id" => group_id,
              "sort" => "id_desc",
    ...
    # puts vk_user_ids
          ...
          vk_user_ids["response"]["items"].each do |vk_user_id|
            ...
            unless maked_likes.include?(vk_user_id)
              ...
              response = Net::HTTP.post_form(uri, {"user_ids" => vk_user_id,
                "fields" => "photo_id",
                "access_token" => bot.access_token,
                "v" => "5.62"})
    ...
    # puts response
              ...
              if photo_id
                ...
                response = Net::HTTP.post_form(uri, {"type" => "photo",
                  "owner_id" => vk_user_id,
                  "item_id" => photo_id,
                  "access_token" => bot.access_token,
                  "v" => "5.62"})
    ...
    # puts "photo"
                ...
                response = Net::HTTP.post_form(uri, {"owner_id" => vk_user_id,
                  "count" => 1,
                  "access_token" => bot.access_token,
                  "v" => "5.62"})
                response = JSON.parse(response.body)
    # puts response
                if response["response"]
                  if response["response"]["items"].any?
                    ...
                    uri = URI.parse("https://api.vk.com/method/likes.add")
                    response = Net::HTTP.post_form(uri, {"type" => "post",
                      "owner_id" => vk_user_id,
  ...
  def self.like_track(bot, vk_user_id, group_id)
    like_track = bot.task.like_trakings.build(vk_user_id: vk_user_id,
      offset: bot.offset_change,
